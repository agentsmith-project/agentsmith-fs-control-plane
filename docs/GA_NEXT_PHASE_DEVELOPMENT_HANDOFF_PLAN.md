...
| --- | --- | --- | --- | --- |
| Package 0: Evidence Manifest Seed | 现有 manifest baseline、唯一 gate 脚本 | 最终 manifest 字段和 claim taxonomy 前置，包括 `claim_id`、`subclaim_id`、`acceptance_id`、`risk_id`、`capability_id`、`evidence_type`、`evidence_profile`、`default_mode`、`fixture_enabled_mode`、`optional_gated`、`required`、`expected_runtime`、`scope`、`command`、`anchors`、`pass_criteria`、`release_intent`、`final_acceptance_selector`、`claimed_optional_capabilities`、`seed_gap_policy`、`rollback_rollforward_policy_ref`、retry/flake policy 字段或引用；verifier negative seed | gate 能识别缺 claim、缺 required evidence、profile 混用、doc-only high-risk、命令缺失或命令不可 repo-local 运行、selector 缺失/多份/digest 不匹配、seed selector 被用于 final | 只建立证据骨架；不声称现有 baseline 已覆盖最终 GA，也不把字段留到 Package 5 才补。 |
...
- Package 0 必须固定 authoritative final selector 输入位置：
  ...
  缺失、多份 authoritative selector、selector digest 与 manifest/report 不匹配，以及
...
- final selector 的 authoritative input path 固定为
  ...
  多份 authoritative selector、selector digest 与 manifest/generated report 不匹配，以及
...
- release intent/manifest selector 字段必须 repo-local、可 diff、可审计，至少表达：
  ...
  - `release_artifact_id`: 由 git revision、manifest digest、schema/migration set、generated report digest
    组合出的稳定 identity。
...
selector digest、command digest、started/ended/duration、exit code、artifact digest 和 redaction
status。report path 可以在 Package 0 seed 中先固定；P5 只补齐内容和 gate wiring，不在末尾临时换目录结构。
...
| `CLAIM_RELEASE_ARTIFACT_IDENTITY` | release artifact identity 可机器追踪 | git revision + manifest digest + generated report digest + schema/migration set + workflow entrypoint evidence | `release_artifact_id` 必须稳定、可复算；不能用手填版本号或真实 artifact 截图替代。 |
...
| `CLAIM_RELEASE_ARTIFACT_IDENTITY` | `SUBCLAIM_ARTIFACT_MANIFEST_DIGEST` / `ACCEPT_RELEASE_MANIFEST_DIGEST_PINNED` | manifest digest、final selector digest、generated report digest | digest 不可复算或 selector/report 与 manifest 不匹配。 |
...
- generated report 只有 Markdown 摘要，没有机器可读 JSON、selector digest、artifact digest 或 evidence artifact layout。
...
- release artifact identity 可追溯到 git revision、manifest digest、schema/migration set 和
  generated report digest，并映射到 `CLAIM_RELEASE_ARTIFACT_IDENTITY`。
