...
- `idle` recovery status, including the JVS shape with no `restore_state` and
  ...
  durable preflight marker; during timeout reconciliation it should persist a
  retryable failure rather than move the operation to manual intervention.
...
marker. If timeout reconciliation also finds idle/no-pending, AFSCP records a
retryable restore-preview failure because there is no JVS side effect to adopt
or discard. After preview side effects are present, AFSCP may adopt or discard a
...
multiple plans, mismatched plan IDs, unknown blocking state, or malformed
recovery status require AFSCP operator intervention unless the matching discard
operation is explicitly running. Matching `stale_restore_preview` during
restore-run is the typed `RESTORE_PREVIEW_STALE` path and must persist
stale/blockers on the durable restore plan before strong preview metadata
validation can turn it into a mismatch.
