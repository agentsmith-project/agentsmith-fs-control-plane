...
multiple plans, mismatched plan IDs, or malformed recovery status require AFSCP
operator intervention unless the matching discard operation is explicitly
running. Matching `stale_restore_preview` during restore-run is the typed
`RESTORE_PREVIEW_STALE` path and must persist stale/blockers on the durable
restore plan.
