...
- Disabling a namespace rejects repo create, save point create, restore preview, restore-run, template create, template clone, export create, workload mount binding create, and other new mutating/session issuance operations.
- A matching `restore-preview:discard` for an already pending restore plan is a narrow cleanup exception while caller auth and the namespace binding remain valid, the binding is active, the repo is active, and lifecycle/recovery fences are clear. It exists only to clear the matching JVS active plan and does not permit new restore preview or restore-run intake.
- Health inspection, operation inspection, and audit inspection may continue for authorized operator roles. Any operator or break-glass exception for restore preview or restore-run is a future independent capability, not current GA behavior for the namespace-bound restore APIs.
