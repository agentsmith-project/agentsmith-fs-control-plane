...
func exportRevokeRequestFixture(now time.Time) exportaccess.RevokeRequest {
	...
	record := operations.OperationRecord{
		...
		Attempt:             1,
		IdempotencyScope:    operations.NewIdempotencyScope("product-caller", "ns_123", operations.OperationExportRevoke, "idem_revoke_export").String(),
...
func exportReconcileRequestFixture(now time.Time, status sessionstate.ExportStatus) exportaccess.ReconcileRequest {
	...
	record := operations.OperationRecord{
		...
		Attempt:             1,
		IdempotencyScope:    operations.NewIdempotencyScope("export-gateway", "ns_123", operations.OperationExportSessionReconcile, "idem_reconcile_export").String(),
		IdempotencyKey:      "idem_reconcile_export",
		RequestHash:         "sha256:export-reconcile",
		CorrelationID:       "corr_export",
		CallerService:       "export-gateway",
