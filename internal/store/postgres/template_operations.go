...
func (store *Store) CommitTemplateCreateSucceededWithLease(ctx context.Context, template resources.Repo, sourceRepoID, sourceSavePointID, cloneHistoryMode string, sanitized operations.SanitizedOperationRecord, owner string, now time.Time, event audit.Event) (resources.Repo, operations.OperationRecord, error) {
	...
	return store.commitTemplateRepoSuccess(ctx, template, record, owner, now, event, "template create success commit", templateCreateSuccessCommitWithLeaseSQL(), sourceRepoID, sourceSavePointID, cloneHistoryMode)
}
...
func (store *Store) CommitTemplateCloneSucceededWithLease(ctx context.Context, repo resources.Repo, sanitized operations.SanitizedOperationRecord, owner string, now time.Time, event audit.Event) (resources.Repo, operations.OperationRecord, error) {
	...
	return store.commitTemplateRepoSuccess(ctx, repo, record, owner, now, event, "template clone success commit", templateCloneSuccessCommitWithLeaseSQL(), "", "", "")
}
...
func (store *Store) commitTemplateRepoSuccess(ctx context.Context, repo resources.Repo, record operations.OperationRecord, owner string, now time.Time, event audit.Event, label, query, sourceRepoID, sourceSavePointID, cloneHistoryMode string) (resources.Repo, operations.OperationRecord, error) {
	...
	args = append(args, sourceRepoID, sourceSavePointID, cloneHistoryMode)
	args = append(args, record.SessionFenceID)
...
func templateCreateSuccessCommitWithLeaseSQL() string {
	return templateSuccessCommitWithLeaseSQL("template_create", operations.OperationPhaseTemplateCreateWriterFenced, true)
}
