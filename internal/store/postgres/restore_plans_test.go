...
func TestCreatePendingRestorePlanInsertsLifecycleSourceOfTruth(t *testing.T) {
	...
	assertSQLContainsInOrder(t, exec.query,
		...
		"fence_marker", "summary_json", "blockers_json", "stale",
	...
	wantArgs := []any{
		...
		"sp_002",
		"sha256:preview-base",
		"preview_fence_op_preview01",
		mustMarshalJSONForTest(map[string]any{
			"added":       map[string]any{"count": 1, "samples": []string{"src/new.ts"}},
			"changed":     map[string]any{"count": 1, "samples": []string{"docs/readme.md"}},
			"removed":     map[string]any{"count": 1, "samples": []string{"tmp/cache.txt"}},
			"destructive": true,
		}),
		mustMarshalJSONForTest([]map[string]any{}),
		false,
...
func restorePlanRowValues(plan restoreplan.Plan) []any {
	return []any{
		...
		plan.Generation,
		plan.FenceMarker,
		mustMarshalJSONForTest(restoreplan.SummaryMap(plan.Summary)),
		mustMarshalJSONForTest(restoreplan.BlockersList(plan.Blockers)),
		plan.Stale,
		string(plan.Status),
		plan.CreatedAt,
		plan.UpdatedAt,
