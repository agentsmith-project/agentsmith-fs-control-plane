...
func TestOperationWriterContractAcceptsOnlySanitizedPersistenceRecords(t *testing.T) {
	...
	if got, ok := fake.record.InputSummary["command"]; ok {
		t.Fatalf("store write retained forbidden command field: %#v", got)
	}
	renderedInput, err := json.Marshal(fake.record.InputSummary)
	if err != nil {
		t.Fatalf("marshal sanitized input summary: %v", err)
	}
	if rendered := strings.ToLower(string(renderedInput)); strings.Contains(rendered, "store-token-secret") {
		t.Fatalf("store write received unsanitized input summary: %s", rendered)
	}
