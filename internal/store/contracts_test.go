...
func TestRestorePreviewOperationRecoveryStoreContractCommitsPlanOperationAndAuditTogether(t *testing.T) {
	...
	fake.record = operations.OperationRecord{
		...
		LeaseOwner:       "worker-a",
		LeaseExpiresAt:   &leaseExpiresAt,
		IdempotencyScope: operations.NewIdempotencyScope("caller-alpha", "ns_alpha01", operations.OperationRestorePreview, "idem_preview").String(),
		IdempotencyKey:   "idem_preview",
		RequestHash:      operations.RequestHash("sha256:restore-preview"),
		CallerService:    "caller-alpha",
...
func TestRestoreRunOperationRecoveryStoreContractOwnsFencePlanOperationAndAudit(t *testing.T) {
	...
	fake.record = operations.OperationRecord{
		...
		IdempotencyScope: operations.NewIdempotencyScope("caller-alpha", "ns_alpha01", operations.OperationRestoreRun, "idem_run").String(),
	...
	event := audit.NewEvent(audit.Event{EventID: "audit-run", Type: audit.EventTypeRestoreRun, Time: now, OperationID: "op_restore_run01", CallerService: "caller-alpha", CorrelationID: "corr-run", AuthorizedActor: audit.Actor{Type: "system", ID: "svc-alpha"}, Resource: audit.Resource{Type: "repo", ID: "repo_alpha01", NamespaceID: "ns_alpha01"}, Outcome: audit.OutcomeSucceeded, Reason: "restore_run_committed"})
...
func TestIdempotencyStoreContractRequiresAtomicCreateOrReuseBoundary(t *testing.T) {
	...
	spec := operations.QueuedOperationSpec{
		...
		Resource:        operations.ResourceRef{Type: "repo", ID: "repo_unit"},
...
func TestResourceStoresContractCoverControlPlaneMetadataOnly(t *testing.T) {
	...
	binding := resources.NamespaceVolumeBinding{
		...
		AllowedCallers:    []resources.AllowedCaller{{CallerService: "caller-alpha", Roles: []resources.CallerRole{resources.CallerRoleRepoAdmin}}},
...
func TestAuditSinkContractAcceptsAppendOnlyAuditEvents(t *testing.T) {
	...
	event := audit.NewEvent(audit.Event{
		...
		CorrelationID:   "corr-1",
		OperationID:     "op_alpha",
		Resource:        audit.Resource{Type: "repo", ID: "repo_unit", Path: "/payload --token audit-path-token"},
		Outcome:         audit.OutcomeSucceeded,
